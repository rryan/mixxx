...
void WaveformRendererHSV::draw(QPainter* painter,
    ...
    for (int x = 0; x < m_waveformRenderer->getWidth(); ++x) {
        ...
        for (int i = visualIndexStart;
            ...
            maxLow[0] = math_max(maxLow[0], static_cast<int>(waveformData.filtered.low));
            maxLow[1] = math_max(maxLow[1], static_cast<int>(waveformDataNext.filtered.low));
            maxMid[0] = math_max(maxMid[0], static_cast<int>(waveformData.filtered.mid));
            maxMid[1] = math_max(maxMid[1], static_cast<int>(waveformDataNext.filtered.mid));
            maxHigh[0] = math_max(maxHigh[0], static_cast<int>(waveformData.filtered.high));
            maxHigh[1] = math_max(maxHigh[1], static_cast<int>(waveformDataNext.filtered.high));
            ...
            switch (m_alignment) {
                case Qt::AlignBottom :
                    painter->drawLine(
                        ...
                        x, m_waveformRenderer->getHeight() - static_cast<int>(heightFactor*static_cast<float>(math_max(maxAll[0],maxAll[1]))));
                    break;
                case Qt::AlignTop :
                    painter->drawLine(
                        x, 0,
                        x, static_cast<int>(heightFactor*static_cast<float>(math_max(maxAll[0],maxAll[1]))));
                    break;
                default :
                    painter->drawLine(
                        x, static_cast<int>(halfHeight-heightFactor*static_cast<float>(maxAll[0])),
                        x, static_cast<int>(halfHeight+heightFactor*static_cast<float>(maxAll[1])));
            }
        }
    }
