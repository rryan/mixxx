...
EngineBuffer::EngineBuffer(const char * _group, ConfigObject<ConfigValue> * _config,
    ...
    m_pReader = new CachingReader(m_group, _config);
    ...
    ControlPushButton* pPlayButton = new ControlPushButton(
        ConfigKey(m_group, "play"));
    pPlayButton->setButtonMode(ControlPushButton::TOGGLE);

    m_playButton = pCallbackControlManager->addControl(pPlayButton, 1);
    ...
    CallbackControl* pStartPlayButton =
            pCallbackControlManager->addControl(
                new ControlPushButton(ConfigKey(m_group, "start_play")), 1);
    ...
    CallbackControl* pStartStopButton =
        ...
        ConfigKey(m_group, "start_stop")), 1);
    ...
    CallbackControl* pStopButton = pCallbackControlManager->addControl(
        new ControlPushButton(ConfigKey(m_group, "stop")), 1);
    ...
    CallbackControl* pStartButton = pCallbackControlManager->addControl(
        new ControlPushButton(ConfigKey(m_group, "start")), 1);
    ...
    CallbackControl* pEndButton = pCallbackControlManager->addControl(
        new ControlPushButton(ConfigKey(m_group, "end")), 1);
    ...
    CallbackControl* pEjectButton = pCallbackControlManager->addControl(
        new ControlPushButton(ConfigKey(m_group, "eject")), 1);
    pEjectButton->setParent(this);
    connect(pEjectButton, SIGNAL(valueChanged(double)),
            this, SLOT(slotEjectTrack(double)),
            Qt::DirectConnection);

    ControlPushButton* pSlipButton = new ControlPushButton(ConfigKey(m_group, "slip_enabled"));
    pSlipButton->setButtonMode(ControlPushButton::TOGGLE);
    m_pSlipButton = pCallbackControlManager->addControl(pSlipButton, 1);
    ...
    m_pSlipPosition = pCallbackControlManager->addControl(
        new ControlObject(ConfigKey(m_group, "slip_playposition")), 1);

    // Actual rate (used in visuals, not for control)
    m_rateEngine = pCallbackControlManager->addControl(
        new ControlObject(ConfigKey(m_group, "rateEngine")), 1);

    // BPM to display in the UI (updated more slowly than the actual bpm)
    m_visualBpm = pCallbackControlManager->addControl(
        new ControlObject(ConfigKey(m_group, "visual_bpm")), 1);

    // Slider to show and change song position these bizarre choices map
    // conveniently to the 0-127 range of midi
    m_playposSlider = pCallbackControlManager->addControl(
        new ControlLinPotmeter(ConfigKey(m_group, "playposition"),
    ...
    m_visualPlaypos = pCallbackControlManager->addControl(
        new ControlPotmeter(ConfigKey(m_group, "visual_playposition"),
                            kMinPlayposRange, kMaxPlayposRange), 1);

    // Signals when we are at the end of file during playback.
    m_iTrackLoading = 0;

    ControlPushButton* pRepeatButton = new ControlPushButton(ConfigKey(m_group, "repeat"));
    ...
    m_pTrackSamples = pCallbackControlManager->addControl(
        new ControlObject(ConfigKey(m_group, "track_samples")), 1);
    m_pTrackSampleRate = pCallbackControlManager->addControl(
        new ControlObject(ConfigKey(m_group, "track_samplerate")), 1);

    // Quantization Controller for enabling and disabling the
    // quantization (alignment) of loop in/out positions and (hot)cues with
    // beats.
    addControl(new QuantizeControl(m_group, pEngineState));

    // Create the Loop Controller
    m_pLoopingControl = new LoopingControl(m_group, pEngineState);
    addControl(m_pLoopingControl);

#ifdef __VINYLCONTROL__
    // If VinylControl is enabled, add a VinylControlControl. This must be done
    // before RateControl is created.
    addControl(new VinylControlControl(m_group, pEngineState));
#endif

    // Create the Rate Controller
    m_pRateControl = new RateControl(m_group, pEngineState);
    addControl(m_pRateControl);

    m_fwdButton = pCallbackControlManager->getControl(ConfigKey(m_group, "fwd"));
    m_backButton = pCallbackControlManager->getControl(
        ConfigKey(m_group, "back"));

    // Create the BPM Controller
    m_pBpmControl = new BpmControl(m_group, _config, pEngineState);
    ...
    ControlPushButton* pKeylockButton =
            new ControlPushButton(ConfigKey(m_group, "keylock"));
...
void EngineBuffer::setNewPlaypos(double newpos) {
    ...
    m_playposSlider->set(m_filepos_play / m_file_length_old);
...
void EngineBuffer::slotControlSeek(double change) {
    ...
    double min_playpos = kMinPlayposRange * m_file_length_old;
    double max_playpos = kMaxPlayposRange * m_file_length_old;
...
void EngineBuffer::slotLoadTrackFromGuiThread(TrackPointer pTrack, bool play) {
