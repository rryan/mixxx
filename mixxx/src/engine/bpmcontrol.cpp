...
BpmControl::BpmControl(const char* _group,
    ...
    // Get controls from LoopingControl.
    m_pLoopEnabled = pControlManager->getControl(
        ConfigKey(_group, "loop_enabled"));
    m_pLoopStartPosition = pControlManager->getControl(
        ConfigKey(_group, "loop_start_position"));
    m_pLoopEndPosition = pControlManager->getControl(
